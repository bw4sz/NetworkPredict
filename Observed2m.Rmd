...
fl.morph[fl.morph$Group.1 %in% "Fuchsia macrostigma","TotalCorolla"]<-fl.morph[fl.morph$Group.1 %in% "Fuchsia macrostigma","EffectiveCorolla"]
...
```{r}
indatraw<-indatraw %>% group_by(Hummingbird,Iplant_Double,ID,Day,Transect_R) %>% summarize(Yobs=max(Yobs),Time=unique(Time),Month=unique(Month),Year=unique(Year),ele=mean(ele),DateP=unique(DateP),Survey_Type=unique(Survey_Type))
```
