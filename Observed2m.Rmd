...
#plot results by 
ms<-melt(table(sflowers$R,sflowers$Transect_R))

ggplot(ms,aes(x=Var2,y=value,col=Var1)) + geom_point(size=4) + labs(x="Elevation Range",y="Months",col="Resource Range") + scale_y_continuous(limits =c(0,20)) + theme_bw()
```
...
ggplot(fll,aes(x=Corolla,fill=R)) + geom_density(alpha=0.8) + scale_fill_manual(labels=c("High","Medium","Low"),values=c("Black","Grey40","grey90")) + labs(fill="Resource Availability") + theme_bw() + labs(x="Corolla Length (mm)") + facet_wrap(~Transect_R,scales="free")


ggsave("Figures/TraitDensity.jpeg",height=4,width=8,dpi=500)
```
