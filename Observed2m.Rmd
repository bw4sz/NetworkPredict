...
qthresh<-indat %>% group_by(Hummingbird) %>% summarize(UThresh=quantile(Used_Flowers,0.5))
...
#index resources
resourceMatrix<-indat %>% group_by(jBird,jPlant,jTime) %>% summarize(v=mean(BAll_Flowers))  %>% acast(jBird ~ jPlant ~ jTime,value.var='v',fill=0)
