  ...
  for(transect in trans){
    ...
    pres<-a %>% filter(Transect_R %in% transect,DateP %in% Date) %>% distinct(Iplant_Double) %>% .$Iplant_Double
      ...
      for (plant in pres){
        ...
        add_absences<-data.frame(Hummingbird=predh,Iplant_Double=plant,ID=t,DateP=Date,Yobs=0)
...
indat<-rbind_all(absences)

#Day level
#add day ID
sdat<-split(indat,list(indat$ID),drop = T)
