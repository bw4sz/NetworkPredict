...
ggplot(data=spe,aes(x=trait)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=resources),alpha=0.5) + geom_line(aes(y=mean,group=resources),size=.5) + theme_bw() + ylab("Daily Interaction Rate")+ xlab("Difference between Bill and Corolla Length (cm)") + facet_wrap(~Hummingbird,scales="free",ncol=4) + geom_point(data=indat,aes(x=Traitmatch,y=Yobs),size=1.2) + scale_fill_manual("Resource Availability",values=c("Grey70","Grey40","Black"))
ggsave("Figures/SpeciesRegression.jpeg",height=7,width=12)
...
```{r,eval=F}
dmat<-pars_detect %>% filter(par %in% c("ynew","E")) %>% dcast(...~par,value.var="estimate") %>% group_by(ynew) %>% summarize(E=mean(E),lower=quantile(E,0.05),upper=quantile(E,0.95))

ggplot(dmat,aes(x=Yobs,y=E,ymin=lower,ymax=upper)) + geom_point(size=4) + geom_linerange(alpha=0.5,size=1) + coord_flip() + labs(color="N-mixture Model",y="Average Discrepancy") + theme_bw()
ggsave("Figures/SpeciesDiscrepancy.jpeg",height=7,width=8)
```
