...
#Parameters to track
ParsStage <- c("alpha","beta1","beta2","beta3","alpha_mu","alpha_sigma","beta1_sigma","beta1_mu","beta2_mu","beta2_sigma","beta3_mu","beta3_sigma","dtrans","dcam","dprior_cam","dprior_trans","ynew_transect","ynew_camera")
...
As overall distributions

```{r}
ggplot(species.mean[species.mean$phi>0.5,]) + geom_boxplot(aes(y=TotalCorolla,x=as.factor(r)),alpha=0.7)

#merge names
species.traj<-merge(species.traj,jagsIndexBird)
species.traj<-merge(species.traj,jagsIndexPlants)

#get corolla sizes
species.traj<-merge(species.traj,fl.morph,by.x="Iplant_Double", by.y="Group.1")

all.mean<-species.traj %>% group_by(jPlant,r) %>% summarize(TotalCorolla=unique(TotalCorolla),phi=mean(y),phi_low=quantile(y,0.05),phi_high=quantile(y,0.95))

ggplot(all.mean) + geom_ribbon(alpha=0.4,aes(x=TotalCorolla,ymin=phi_low,ymax=phi_high,fill=as.factor(r))) + theme_bw() +  ggtitle("Niche Breadth") + ylab("Probability of Interaction") + xlab("Corolla Length (mm)") + scale_fill_discrete("Resource Availability") + ylim(0,1)
