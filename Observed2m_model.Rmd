...
Dat<-list(
  ...
  if(newModel){
    system.time(
      m2<-jags.parallel(data=Dat,parameters.to.save =ParsStage,inits=InitStage,model.file="Bayesian/NmixturePoissonRagged2m.jags",n.thin=8,n.iter=400000,n.burnin=400000-4000,n.chains=2,DIC=F)
      )
  }
```
...
###Chains
ggplot(pars_detect[pars_detect$par %in% c("alpha","beta1","beta2","beta3","tauE"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(par~species,scale="free") + theme_bw() + labs(col="Chain") + ggtitle("Species Level")
```
...
```{r}
castdf<-dcast(pars_detect[pars_detect$par %in% c("beta1","beta2","beta3","alpha","tauE"),], species +Chain + Draw~par,value.var="estimate")
