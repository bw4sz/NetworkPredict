...
$$ YTransect_{i,j,k,d} \sim Bernoulli(p_{i,j,k},\omega_{Transect}) $$

$$ YCamera_{i,j,k,d} \sim Bernoulli(p_{i,j,k},\omega_{Camera}) $$
$$ \omega_{Camera} <- \phi_{Camera} * EffortCamera_k $$
$$ \omega_{Transect} <- \phi_{Transect}* EffortTransect_k $$
$$ p_{i,j,k} \sim Binomial(\rho_{i,j,k}) $$
...
$$\mu_\beta_1 \sim N(0,0.0001)$$
$$\mu_\beta_2} \sim N(0,0.0001)$$
$$\mu_\beta_3 \sim N(0,0.0001)$$
