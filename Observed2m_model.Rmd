...
ggplot(indat[indat$Yobs==1,],aes(x=Traitmatch,fill=as.factor(BAll_Flowers))) + facet_wrap(~Hummingbird,ncol=4,scales="free") + geom_density(alpha=.7) + scale_fill_manual("Resource Availability",values=c('blue','red'))

m<-glmer(data=indat[,],Yobs~Traitmatch*scaledR+(1|Hummingbird),family="poisson")
summary(m)
indat$pred<-predict(m,type="response")
ggplot(indat,aes(x=Traitmatch)) + geom_point(aes(y=Yobs)) + geom_line(aes(y=pred,col=as.factor(BAll_Flowers)))
```
...
Dat<-list(
  ...
  if(newModel){
    system.time(
      m2<-jags.parallel(data=Dat,parameters.to.save =ParsStage,inits=InitStage,model.file="Bayesian/NmixturePoissonRagged2m.jags",n.thin=4,n.iter=100000,n.burnin=98000,n.chains=2,DIC=F)
      )
  }
```
